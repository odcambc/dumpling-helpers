...
export function ImportButton({ onImport }: Props) {
  ...
  async function processFiles(files: FileList | File[]) {
    ...
    for (const file of fileArr) {
      ...
      try {
        if (lower.endsWith('.yaml') || lower.endsWith('.yml')) {
          ...
          result.samples = importSamplesCsv(text)
          loadedNames.push(file.name)
        } else {
    ...
    if (result.config || result.samples) onImport(result)
  }
      ...
      {banner && (
        <div
          className={cn(
            ...
            banner.type === 'success' ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-600',
          )}
        >
