...
import { StepBarcode } from '@/components/wizard/StepBarcode'
import { StepCalling } from '@/components/wizard/StepCalling'
...
const STEPS: { label: string; title: string }[] = [
  ...
  { label: 'Barcode', title: 'Barcode & ORF' },
  { label: 'Calling', title: 'Variant calling' },
...
export default function App() {
  ...
  const form = useForm<ConfigFormValues>({
    ...
    // output (ConfigFormValues) has them required. configDefaults supplies every
    // field, so input === output at runtime; cast the resolver to the output type.
  ...
  async function handleDownload() {
    ...
    try {
      ...
      zip.file('experiments.csv', buildSamplesCsv(rows))
      const blob = await zip.generateAsync({ type: 'blob' })
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
      a.download = `${config.experiment || 'stromboli'}-config.zip`
      a.click()
        ...
        <div className="flex-1 overflow-y-auto p-8">
          <div className="max-w-xl">
            <form>
              ...
              {step === 2 && <StepBarcode form={form} />}
              {step === 3 && <StepCalling form={form} />}
              {step === 4 && <SampleTable rows={rows} onChange={setRows} />}
            </form>
          </div>
        </div>
          ...
          {(['structure', 'files'] as const).map((t) => (
            <button
              ...
              className={cn(
                ...
                previewTab === t ? 'bg-brand-light text-brand-dark' : 'text-gray-500 hover:bg-gray-50',
              )}
            >
