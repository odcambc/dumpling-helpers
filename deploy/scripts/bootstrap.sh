...
echo "  2. Copy deploy/.env.example to /etc/caddy/Caddyfile.env, fill in the three hosts."
echo "  3. Install the vhost snippet, then add 'import /etc/caddy/conf.d/*.caddy' to /etc/caddy/Caddyfile:"
echo "       mkdir -p /etc/caddy/conf.d"
echo "       cp $REPO_DIR/deploy/caddy/dumpling-helpers.caddy /etc/caddy/conf.d/"
echo "  4. First deploy:"
